    ...
    MapView,
      ...
      function handleCsvParsingComplete(results) {
        ...
        view.whenLayerView(canvasFlowmapLayer).then(function (canvasFlowmapLayerView) {
          ...
          $("#loading").fadeOut();
          $("#detail").html('Please choose departure and destination.<br />Click "Shortest Path" to search the shortest path(s).<br />Click "All Paths" to search all paths.');
          ...
          $('#bt3').click(function () {
            ...
            if ($('#departure').find(':selected').html() == 'IAH' || $('#destination').find(':selected').html() == 'IAH') {
              ...
              $('#detail').html('There are total ' + paths.length + ' paths from ' + getCity(startcode) + '(' + startcode + ')' + ' to ' + getCity(endcode) + '(' + endcode + ')' + '<br>')
